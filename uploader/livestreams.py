...
class LiveStreamUploader:
    ...
    async def upload_live_videos(cls):
        LogManager.log_upload_live(
            f"Monitoring {cls.Live_UploadQueue_Dir} For Published Live Streams"
        )
        if cls.upload_live_videos_lock.locked():
            LogManager.log_upload_live(
                "upload_live_videos is already running. Skipping this invocation."
            )
            return
            ...
            while True:
                try:
                    ...
                    for file in video_files:
                        ...
                        LogManager.log_upload_live(
                            f"Discovered new file: {file} for upload processing."
                        )

                        filename = os.path.splitext(file)[
                            0
                        ]  # Extract file name without extension
                        if filename.lower().endswith("am") or filename.lower().endswith(
                            "pm"
                        ):
                            # Run uploads concurrently for this file
                            LogManager.log_upload_live(
                                f"Starting upload of file: {file} to video hosts"
                            )
                            try:
                                ...
                                LogManager.log_upload_live(
                                    f"Exception while uploading {file}: {upload_exc}\n{traceback.format_exc()}"
                                )
                            ...
                            try:
                                ...
                                os.makedirs(
                                    cls.Live_CompletedUploads_Dir, exist_ok=True
                                )
                                ...
                                LogManager.log_upload_live(
                                    f"Moved {file} to completed uploads."
                                )
                            except Exception as move_exc:
                                LogManager.log_upload_live(
                                    f"Failed to move {file} to completed uploads: {move_exc}\n{traceback.format_exc()}"
                                )
                            ...
                            try:
                                LogManager.archive_logs(
                                    filename,
                                    "_Archived_LiveStream_Logs",
                                    archive_log_files,
                                )
                            except Exception as archive_exc:
                                LogManager.log_upload_live(
                                    f"Failed to archive logs for {filename}: {archive_exc}\n{traceback.format_exc()}"
                                )
                    ...
                    for file in other_files:
                        LogManager.log_upload_live(
                            f"file: {file} has the wrong file extension"
                        )

                except Exception as e:
                    LogManager.log_upload_live(
                        f"Exception in upload_live_videos: {e}\n{traceback.format_exc()}"
                    )
                finally:
