...
class CaptionsDownloader:
        ...
        while True:
            async with cls._monitor_execution_lock:
                if cls._download_execution_lock.locked():
                    ...
                    await cls.download_playlist_json(
                        cls.video_playlist_command, cls.video_json
                    )
                    await cls.download_playlist_json(
                        cls.shorts_playlist_command, cls.shorts_json
                    )
            ...
            # --- Waiting section ---
            LogManager.log_download_captions(
                f"Caption Index Proccess completed. Sleeping for 1 hour."
            )
    ...
    async def download_playlist_json(cls, playlist_command, outputfile):
        ...
        if process.returncode == 0:
            LogManager.log_download_captions(
                f"Successfully Updated Playlist {outputfile}"
            )
        else:
            LogManager.log_download_captions(
                f"Failed to update playlist. Error: {stderr.decode()}"
            )
    ...
    async def populate_hascaptions_field(cls):
        ...
        if len(video_ids) != 0:
            ...
            LogManager.log_download_captions(
                f"This make quite some time for large channels.."
            )
    ...
    async def check_captions(cls, video_id: str) -> Tuple[str, bool]:
        def run_subprocess():
            try:
                ...
                has_captions = bool(
                    metadata.get("automatic_captions") or metadata.get("subtitles")
                )
                LogManager.log_download_captions(
    ...
    async def download_caption_for_video(cls, video_id: str, filepath: str) -> bool:
        ...
        if process.returncode != 0:
            LogManager.log_download_captions(
                f"yt-dlp error for video {video_id}: {stderr.decode()}"
            )
            return False
        ...
        try:
            ...
            LogManager.log_download_captions(
                f"Post-processing error for video {video_id}: {str(e)}"
            )
            return False
